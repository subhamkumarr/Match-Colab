...
export default function ProfileCard({ profile }: ProfileCardProps) {
  ...
  return (
    <>
      <div
        className="rounded-lg border-2 border-orange-400 bg-white transition-all duration-300 hover:shadow-xl hover:border-orange-500 cursor-pointer flex flex-col w-full h-full group"
        onClick={() => setOpenModal(true)}
      >
        {/* Avatar & Name */}
        <div className="flex flex-col items-center pt-8 pb-6 transition-all duration-300 group-hover:bg-orange-50">
          <div className="relative w-24 h-24 rounded-full overflow-hidden mb-4 bg-orange-100 border-4 border-orange-100 transition-all duration-300 group-hover:border-orange-200 group-hover:scale-105">
            <Image
              src={profile.avatar}
              alt={profile.name}
              fill
        ...
        {/* Divider strip */}
        <div className="h-[12px] bg-orange-50 border-y border-orange-200/50 w-full transition-colors duration-300 group-hover:bg-orange-100 group-hover:border-orange-300/50"></div>
          ...
          {profile.hobbies.slice(0, 2).map((hobby, idx) => (
            <div
              ...
              className={`flex flex-col mb-3 ${
                idx === 0 ? 'pb-3 border-b border-orange-800 transition-colors duration-300 group-hover:border-orange-500' : ''
              }`}
            >
              <span className="font-medium text-gray-800 transition-colors duration-300 group-hover:text-orange-900">
                {hobby.title}:
              </span>
        ...
        <div className="px-5 pt-5 pb-6 text-gray-700 transition-colors duration-300 group-hover:bg-orange-50/30">
          <p className="font-medium text-gray-700 transition-colors duration-300 group-hover:text-orange-900">{profile.location}</p>
      ...
      >
        ...
        <div className="p-4 sm:p-6 bg-slate-700 text-white flex items-center relative">
          <div className="relative w-10 h-10 sm:w-14 sm:h-14 rounded-full overflow-hidden mr-3 sm:mr-5 flex-shrink-0">
            <Image 
              src={profile.avatar} 
              alt={profile.name} 
              fill
          ...
          <h3 className="text-xl sm:text-2xl md:text-3xl font-medium text-slate-200 line-clamp-1">
            {profile.name}, <span className="text-slate-300">{profile.age}</span>
          </h3>
          <button
            onClick={() => setOpenModal(false)}
            className="absolute right-4 top-4 sm:right-5 sm:top-5 text-slate-300 hover:text-white transition-colors"
        ...
        <div className="p-0">
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="flex-1 flex flex-col">
              {/* Tagline section */}
              <div className="p-4 sm:p-6 bg-slate-700 text-white">
                <h3 className="text-xl sm:text-2xl font-medium text-orange-400 line-clamp-2">{profile.tagline}</h3>
              </div>
